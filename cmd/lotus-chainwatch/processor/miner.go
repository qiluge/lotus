...
func (p *Processor) storeMinersSectorState(miners []minerActorInfo) (err error) {
	...
	for _, m := range miners {
		sectors, err := p.node.StateMinerSectors(context.TODO(), m.common.addr, nil, true, m.common.tsKey)
		if err != nil {
...
func (p *Processor) updateMinersSectors(ctx context.Context, miners []minerActorInfo) (err error) {
	...
	for _, m := range miners {
		...
		if err != nil {
			log.Debugw("error getting miner sector diff", "miner", m.common.addr, "error", err)
			continue
		}
		if !changed {
			continue
